...
module.exports = (zuoraClient) => ({
	get: (accountUid) =>
		zuoraClient.authenticate().then((headers) =>
			got
				.get(
					`${zuoraClient.serverUrl}/v1/payment-methods/credit-cards/accounts/${encodeURIComponent(
						accountUid
					)}`,
					{
						headers,
	...
	update: (methodId, body) =>
		zuoraClient.authenticate().then((headers) =>
			got
				.put(
					`${zuoraClient.serverUrl}/v1/payment-methods/credit-cards/${encodeURIComponent(
						methodId
					)}`,
					{
						headers,
